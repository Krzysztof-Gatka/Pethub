...
const AnimalEdit: React.FC = () => {
    ...
    useEffect(() => {
        const fetchAnimalData = async () => {
          try {
            const response = await axios.get(`http://localhost:3000/api/animals/raw/${id}`);
            setForm({
                ...response.data,
                birth_date: response.data.birth_date.split('T')[0],
                date_joined: response.data.date_joined.split('T')[0]
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await axios.post(`http://localhost:3000/api/animals/${id}/edit`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
            console.log(response.data);
            alert('Profil zwierzęcia został zapisany!');
            navigate('/shelter/animals')
        } catch (error) {
            console.error('Błąd podczas edycji zwierzęcia:', error);
            alert('Nie udało się edytować profilu zwierzęcia.');
        }
